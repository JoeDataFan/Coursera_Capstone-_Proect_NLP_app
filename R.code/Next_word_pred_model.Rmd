...
predict_next_word <- function(prev.words, model = ngrams.freq.15){
    ...
    #results.dt <- results.dt[order(order)]
    #results.dt <- results.dt[, norm.prob := prob ^ (1/order)]
    #result <- results.dt[order(-prob), pred] %>% as.character(.)
    result <- results.dt[order(-prob)[1], .(base, pred)]
    #result <- result$pred
    #result <- result[, actual.pred := dt[n.rows, pred]]
    #result <- result[, result := actual.pred == pred]
    print(result)
    }

predict_next_word(prev.words = valid.pentagram.base[589])

num.to.test <- 6000:6050
...
model.test %>% summarise(correct.pred.rate = sum(result)/length(result))

input <- str_replace_all(dt[["base"]][n.rows], "_", " ")

    n = 4:1
    patt = sprintf("\\w+( \\w+){0,%d}$", n-1)
    test.input <- data.table(base = stri_extract(input, regex = patt))
    test.input <- test.input[, order := length(test.input$base):1]
    prep.input <- test.input[, base := str_replace_all(test.input$base, "\\s", "_") %>% 
        tolower(), with = TRUE] 
    prep.input <- setkey(prep.input, base)
    ...
    #results.dt <- merge(prep.input, model, by = "base")
    results.dt <- model[base %in% prep.input]
    #results.dt <- prep.input %>% 
    #    left_join(model)
    #results.dt <- results.dt[order(order)]
    #results.dt <- results.dt[, norm.prob := prob ^ (1/order)]
    result <- results.dt[prob == max(prob)]
    result <- result[, actual.pred := dt[n.rows, pred]]
    result <- result[, result := actual.pred == pred]
    print(result)
```
...
validate_model <- function(dt = valid.pentagram, n.rows, model = ngrams.freq.15){
#ptm <- proc.time() # start time    
    #dt <- valid.pentagram
    #n.rows <- 100
    #model <- ngrams.freq.15
...
proc.time() - ptm # diff time

ptm <- proc.time() # start time
map(.x = 10030, .f = validate_model)
validate_model(n.rows = 10030)
proc.time() - ptm # diff time


ptm <- proc.time() # start time
merge(prep.input, ngrams.freq.15, by = "base")
    #ngrams.freq.15[base %in% prep.input]
    #ngrams.freq.15[base %in% prep.input]
    #results.dt <- prep.input %>% 
    #    left_join(model)
proc.time() - ptm # diff time

#test.output <- merge(prep.input, ngrams.freq.15, by = "base")
test.output <- ngrams.freq.15[prep.input]
