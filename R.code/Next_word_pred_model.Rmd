...
set.seed(123)
training <- sample(all.text,
                   size = sample.size * length(all.text),
                   replace = FALSE)
valid <- sample(all.text[-which(training %in% all.text)],
                   size = valid.size * length(all.text),
                   replace = FALSE) 
test <- sample(all.text[-which(c(training, valid) %in% all.text)],
