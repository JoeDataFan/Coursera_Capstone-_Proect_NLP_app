...
split_base_pred_freq <- function(dfm.ngram){
    ...
    dt <- dt[ , .SD[which.max(count)], # how could I keep top 2 or 3 counts for each base?
              key = base]
    return(dt)
}
...
corpus.sentences <- corpus(test) %>%
    corpus_reshape(., to = "sentence") 

# Create ngram frequencies data table-------------------------------------------

bigrams.test <- dfm_ngrams(corpus = corpus.sentences, ngrams = 2)
dt.bigram.test <- data.table(base = names(bigrams.test),
                             count = colSums(bigrams.test),
                             row.names = NULL,
